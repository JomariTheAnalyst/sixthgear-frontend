export const companyData = {
  name: "Sixthgear Motosupply",

  purpose: `The purpose of our company is to provide comprehensive motorcycle maintenance, servicing, upgrades, and customization services, and to establish, operate, and manage a professional motorcycle service center catering to both local and international brands.`,

  services: [
    {
      title: "Motorcycle Service Center",
      description:
        "Comprehensive motorcycle maintenance, servicing, upgrades, and customization services for local and international brands.",
    },
    {
      title: "Café & Coffee Shop",
      description:
        "Freshly brewed coffee, pastries, meals, and a selection of non-alcoholic and alcoholic beverages, cocktails, and food. A social hub for motorcycle enthusiasts and the general public.",
    },
    {
      title: "Apparel & Merchandise",
      description:
        "Design, manufacture, distribution, and sale of branded and customized motorcycle-related apparel, clothing, protective gear, and lifestyle merchandise.",
    },
    {
      title: "Events & Community",
      description:
        "Organization and sponsorship of motorcycle-related events, rides, and social gatherings that foster camaraderie among riders and enthusiasts.",
    },
  ],

  servicesOffered: [
    {
      title: "Machine Repair & Routine Service",
      description:
        "Complete engine overhauls, tune-ups, and scheduled maintenance to keep your motorcycle running at peak performance. We handle everything from oil changes to full engine rebuilds.",
    },
    {
      title: "Inspection & Diagnostics",
      description:
        "Thorough inspection and advanced diagnostics of your entire motorbike using modern equipment to identify issues before they become major problems.",
    },
    {
      title: "Suspension Service",
      description:
        "Expert checking, adjustment, and repair of motorcycle suspension systems including forks, shocks, and linkages for optimal ride comfort and handling.",
    },
    {
      title: "Electrical Repairs",
      description:
        "Complete electrical system diagnostics and repairs including wiring, lighting, battery service, and electronic component replacement.",
    },
    {
      title: "Brake & Tire Service",
      description:
        "Professional brake inspection, pad replacement, fluid changes, and tire mounting, balancing, and replacement to ensure your safety on the road.",
    },
    {
      title: "Customization & Modification",
      description:
        "Transform your motorcycle with custom builds, performance upgrades, aesthetic modifications, and personalized accessories to match your style.",
    },
    {
      title: "Carwash and Detailing",
      description:
        "Transform your motorcycle with custom builds, performance upgrades, aesthetic modifications, and personalized accessories to match your style.",
    },
  ],

  businessActivities: [
    "Motorcycle maintenance and servicing",
    "Motorcycle upgrades and customization",
    "Café and coffee shop operations",
    "Apparel and merchandise design and sales",
    "Event organization and sponsorship",
    "Franchising and licensing arrangements",
    "Partnerships and joint ventures",
    "Property acquisition and management",
  ],

  fullPurposeStatement: `The purpose of our company is to provide comprehensive motorcycle maintenance, servicing, upgrades, and customization services, and to establish, operate, and manage a professional motorcycle service center catering to both local and international brands.

In addition, the company aims to operate and manage a café and coffee shop offering freshly brewed coffee, pastries, meals, and a selection of non-alcoholic and alcoholic beverages, cocktails, and food. This space is intended to serve as a social hub for motorcycle enthusiasts and the general public.

The company also seeks to design, manufacture, distribute, and sell branded and customized motorcycle-related apparel, clothing, protective gear, and lifestyle merchandise.

Furthermore, the company intends to organize and sponsor motorcycle-related events, rides, and social gatherings that foster camaraderie among riders and enthusiasts.

To support growth and expansion, the company may engage in franchising, licensing, and similar arrangements, subject to applicable laws and regulations. The company may also enter into partnerships, joint ventures, and collaborations with local and international suppliers, manufacturers, and related businesses to enhance its offerings.

Lastly, the company may acquire, lease, or manage properties necessary for the operation and expansion of the business, subject to applicable laws, excluding financial leasing.`,
}

export type CompanyData = typeof companyData
