                            ...
                            <div className="flex flex-col gap-0.5">
                                {categories.map((service, index) => (
                                    <Link
                                        ...
                                        className={`text-left px-3 py-2 text-sm font-semibold transition-all duration-150 ${activeCategoryIndex === index
                                            ? "text-gray-900 bg-gray-50"
                                            : "text-gray-500 hover:text-gray-900 hover:bg-gray-50"
                                            }`}
                                    >
                                        {service.title.replace(/[\u{1F300}-\u{1F9FF}]/gu, '').trim()}
                                    </Link>
                                ))}
                            </div>
                        ...
                        <div className="col-span-12 lg:col-span-9 pl-4">
                            ...
                            <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
                                <h2 className="text-lg font-black uppercase tracking-tight text-gray-900">
                                    {categories[activeCategoryIndex].title.replace(/[\u{1F300}-\u{1F9FF}]/gu, '').trim()}
                                </h2>
                                <Link
                                    href={`/services#${getSlug(categories[activeCategoryIndex].title)}`}
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                                {categories[activeCategoryIndex].items.slice(0, 6).map((item, i) => {
